    ...
    <>
      <StepShell step={step}>
        <p className="body text-ink-600 mb-6 max-w-[680px]">
          Master 자문은 두 가지 채널이 양쪽으로 함께 굴러갑니다. 사이사이 즉시 응답하는 핫라인과, 정기적으로 깊이 다루는 현장 방문 세션.
        </p>

        {/* Two equal-weight main channels */}
        ...
        <p className="caption mt-6 text-center">
          두 채널은 보완 관계입니다. 핫라인이 일상의 막힘을 풀어준다면, 현장 방문은 제도와 사람을 깊이 다룹니다.
        </p>
      </StepShell>
...
/* ─────────────── Channel 1 — Realtime hotline (chat) ─────────────── */
...
function ChannelHeader({ badge, title, tagline, icon: Icon }: {
  ...
  return (
    <div className="mb-4">
      ...
      <h2 className="h-3 mb-1">{title}</h2>
      <p className="caption text-ink-600">{tagline}</p>
    </div>
  );
}
